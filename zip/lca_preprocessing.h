...
inline void eulerian_walk(FreqDiffTree::Node* node, std::vector<int>& E, std::vector<int>& L, std::vector<int>& R, int depth) {
	E.push_back(node->id);
	L.push_back(depth);
...
inline lca_t* lca_preprocess(FreqDiffTree* t) {
...
inline void general_rmq_preprocess(gen_rmq_t* gen_rmq_prep) {
	...
	FreqDiffTree* cartesian = new FreqDiffTree;
	std::vector<FreqDiffTree::Node*> orig_pos;
	FreqDiffTree::Node* start = cartesian->add_node();
	FreqDiffTree::Node* root = start;
	...
	for (size_t i = 1; i < gen_rmq_prep->v.size(); i++) {
		if (gen_rmq_prep->v[i] >= start->weight) {
			FreqDiffTree::Node* node = cartesian->add_node();
	...
	for (FreqDiffTree::Node* node : orig_pos) {
