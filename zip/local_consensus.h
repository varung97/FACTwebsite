...
void print_bitmask_as_indices(uint bitmask, int len) {
	...
	for (int i = 0; i < len; i++) {
		if (bitmask & (1<<i)) {
			std::cout << FreqDiffTree::taxa_names[i] << " ";
		}
	}
	std::cout << ")" << std::endl;
}
...
void print_tree(uint Lbitmask, std::vector<uint>* components, int** dp_backtrack, FreqDiffTree* tree, FreqDiffTree::Node* node, bool minrs) {
	...
	if (minrs) {
		...
		for (uint i = 0; i < FreqDiffTree::get_taxas_num(); i++) {
			if (singletons & (1<<i)) {
	...
	if (m == 0) {
		...
		FreqDiffTree::Node* new_node = tree->add_node();
...
int*** get_common_triplets(std::vector<FreqDiffTree*>& trees) {
	...
	for (FreqDiffTree* tree : trees) {
		...
		for (int i = 0; i < n; i++) {
			FreqDiffTree::Node* leafi = tree->get_leaf(i);
			for (int j = i+1; j < n; j++) {
				FreqDiffTree::Node* leafj = tree->get_leaf(j);
				int dlcaij = tree->get_node(lca(lca_prep, leafi->id, leafj->id))->depth;
				for (int k = j+1; k < n; k++) {
					FreqDiffTree::Node* leafk = tree->get_leaf(k);
...
FreqDiffTree* minRILC(std::vector<FreqDiffTree*>& trees, bool minrs) {
	...
	FreqDiffTree* tree = new FreqDiffTree();
...
FreqDiffTree* minRLC_exact(std::vector<FreqDiffTree*>& trees) {
	return minRILC(trees, true);
}
FreqDiffTree* minILC_exact(std::vector<FreqDiffTree*>& trees) {
	return minRILC(trees, false);
}

void ahoRec(std::vector<int> indices, int*** triplets, int n, FreqDiffTree* tree, FreqDiffTree::Node* node) {
...
FreqDiffTree* ahoBuild(std::vector<FreqDiffTree*>& trees) {
	int n = trees[0]->get_taxas_num();
	int*** triplets = get_common_triplets(trees);
