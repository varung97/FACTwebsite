#ifndef FREQ_DIFF_TREE_H_
#define FREQ_DIFF_TREE_H_
...
public:
	...
	FreqDiffTree(size_t nodes_num_hint = 0);
	FreqDiffTree(std::string& newick_str);
	FreqDiffTree(FreqDiffTree* other);
	virtual ~FreqDiffTree();
	...
	FreqDiffTree::Node* add_node(int taxa = -1, int label = 0);
...
private:
	...
	FreqDiffTree::Node* build_tree(const char*&);
	void fix_tree_supp(FreqDiffTree::Node* root);
};

class FreqDiffTree::Node {
public:
