...
void build_taxas_ranges_supp(FreqDiffTree::Node* node, taxas_ranges_t* tr) {
	if (node->is_leaf()) {
...
taxas_ranges_t* build_taxas_ranges(FreqDiffTree* tree) {
...
int* get_taxas_ranks(taxas_ranges_t* tr) {
	int* ranks = new int[FreqDiffTree::get_taxas_num()];
