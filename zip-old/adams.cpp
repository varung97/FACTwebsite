      ...
      while(!S.empty()){
        ...
        if(t1.second < T[z].G[t1.first].size()){
          ...
          if(T[z].minL[t1.first] && T[z].size[t1.first] == 1)
            arr[T[z].leaf[t1.first]] = 1;
        }
      }
      for(int i=0;i<(int)V.size();++i) ID[V[i]] += ((arr[V[i]] == 1)?'1':'0');
    }
...
retTree adamsRecurse(set <int> *S){
  ...
  for(int z=0;z<numTrees;++z){
    ...
    while(ai){
      ...
      ++t1.second;
      if(t1.second < (int)A[z].G[t1.first].size()){
        STK[ai++] = t1;
      ...
      for(vector<int>::iterator it=recurLeaf[recurDepth].begin();it!=recurLeaf[recurDepth].end();++it,++cnt){
        ...
        for(int z=0;z<numTrees;++z){
          if(A[z].L_POS[*it] >= A[z].mL[recurBigTree[z][recurDepth]] && A[z].L_POS[*it] <= A[z].mR[recurBigTree[z][recurDepth]])
            sortLeaf[cnt].push_back(1);
          else{
						sortLeaf[cnt].push_back(0);
					}
				}
      }
      for(int i=0;i<cnt;++i) sortArray[i] = i;
      adamsRadixSort(sortArray,sortLeaf,cnt);
      cur = 0;
      for(int i=0;i<cnt;++i)
...
void precomputeLCA(){
  ...
  for(int z=0;z<numTrees;++z){
    ...
    while(ai){
      pair<int,int> t1 = STK[--ai];
      if(t1.second == -1 && T[z].leaf[t1.first] > 0) LCA_POS[z][T[z].leaf[t1.first]] = cnt;
      arr[cnt++] = t1.first;
      ++t1.second;
...
void adamsRadixSort(int *A,vector <int> *S,int L){
  int P[2][L],cnt;
  bool f = 0;
